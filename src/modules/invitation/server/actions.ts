...
export async function actionEvaluatePublishReadiness(
  invitationId: string,
): Promise<ActionResult<import("../publish-readiness").PublishReadinessResult>> {
  try {
    const { requireUser } = await import("@/modules/auth/server/require-user");
    const user = await requireUser();
    const { evaluatePublishReadiness } = await import("./publish-readiness-evaluator");
    const result = await evaluatePublishReadiness(user.id, invitationId);
    return { success: true, data: result };
  } catch (e) {
    return { success: false, error: e instanceof Error ? e.message : "An error occurred", code: "TEMPORARY_ERROR" };
  }
}
